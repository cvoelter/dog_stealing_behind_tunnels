...
demo.data <- read.csv(file = "data/counterbalancing_stealing_tunnels.csv") %>%
  mutate(Subject = fct_recode(as.factor(Subject), Bärli = "B\xe4rli")) %>%
...
#LL: I only count 2 trials
xx<-table(first_choice_data$Subject, first_choice_data$trial)
sum(apply(xx<1, MARGIN=1, FUN=sum)) #which subjects (rows) in the above table have missing trials?
...
```
```{r}
ggplot(data = first_choice_data.agg3, aes(x = condition, y= mean_opaque))+
    geom_bar(stat = "identity", position=position_dodge(.9))+
  geom_errorbar(aes(ymin = mean_opaque - se_opaque, ymax = mean_opaque + se_opaque  ),  position=position_dodge(.9), width=0.4)+
  theme_bw() +
  ylim(0, 1) + 
...
## Behavioral data: firts choice - condition comparison
...
```
In the test condition, the dogs' performance did not deviate significantly from the chance value of 0.5 (z = -0.86, p = 0.391)
```{r}
...
```{r}
#LL:failed attempt 1
#xx.fe.re=fe.re.tab(fe.model="prop.latency~condition+ choice+trial_w_con +first_condition",
#re="(1|Subject)", data=latency_fc_data2)

#latency_fc_data2_model<-xx.fe.re$data
#scale and centre covariate (trial)
#latency_fc_data2_model$z.trial<-(latency_fc_data2_model$trial_w_con-mean(latency_fc_data2_model$trial_w_con))/sd(latency_fc_data2_model$trial_w_con)

#Center factor (condition.test, for rs component, already dummy coded)
#latency_fc_data2_model$condition.test.c=latency_fc_data2_model$condition.test-mean(latency_fc_data2_model$condition.test)
...
boot.res.latency$ci.estimates$fe[1:6,]
...
#LL: failed attempts
# mm1.latency.for.ci<-glmmTMB(prop.latency ~ choice + condition + z.trial + first_condition + 
#             (1|Subject)+
#             (0+condition.c|Subject),
#             family=beta_family, data=latency_fc_data2%>%filter(!is.na(prop.latency)),  control=glmmTMBControl(optCtrl=list(iter.max=100000000, eval.max=100000000)))
# 
# boot.mm1.latency<-boot.glmmtmb(mm1.latency.for.ci, data=as.data.frame(latency_fc_data2%>%filter(!is.na(prop.latency))),
...
# 
# 
# boot.mm1.latency<-boot.glmmtmb(mm1.latency, data=latency_fc_data2,
# nboots=1000, para=T, n.cores="all-1", level=0.95 )
# 
# boot.mm1.latency$ci.estimates$fe
# 
# 
# boot.m1.latency=boot.glmmtmb(m=mm1.latency, data=latency_fc_data2,
...
#### fit LMM LL:the asumption of normally and homogeneously distributed residuals are violated and hence I re-do this analysis fitting a beta model instead. Consider deleting this LMM from the final script
```{r}
mm1.latency.int <- lme4::lmer(
  log_latency ~ choice*condition + z.trial + first_condition +
    (1 + condition.c | Subject),
  data = latency_fc_data,
  REML = FALSE
)

drop1(mm1.latency.int, test="Chisq") # interaction between choice and condition not significant --> interaction is dropped to evaluate the main effects
```
```{r}
mm1.latency <- lme4::lmer(
  log_latency ~ choice+condition + z.trial + first_condition +
    (1 + condition.c | Subject),
  data = latency_fc_data,
  REML = FALSE
...
mm1.latency.reml<-lmerTest::lmer(
  log_latency ~ choice + condition + z.trial + first_condition +
    (1 + condition.c | Subject),
  data = latency_fc_data,
  REML=TRUE)
...
```{r}
length(residuals(mm1.latency))/
(length(fixef(mm1.latency))+
nrow(as.data.frame(summary(mm1.latency)$varcor)))
```
model stability
...
mm1.latency.stab=glmm.model.stab(model.res=mm1.latency, contr=NULL, para=F, data=NULL)

mm1.latency.stab$summary

m.stab.plot(round(mm1.latency.stab$summary[, -1], 3))
```
--> model stable with regard to the fixed effects

Confidence intervals
```{r}
boot.mm1.latency=boot.lmer(mm1.latency,
nboots=1000, para=T, n.cores="all-1", resol=1000, level=0.95)

mm1.latency_boot_ci<-boot.mm1.latency$ci.estimates
```
Effect size
```{r}
library(MuMIn)
r.squaredGLMM(mm1.latency)
```


#### output table - latency
```{r}
model_table_mm1.latency <- bind_cols(as.data.frame(summary(mm1.latency)$coefficients),
                         summary(mm1.latency.reml)$coefficients[,3:5],
                         mm1.latency_boot_ci,
                         mm1.latency.stab$summary[1:5, -1]) %>% 
  dplyr::select(Estimate, SE = `Std. Error`, df, t_value = `t value...5`, p = `Pr(>|t|)`, LowerCI = X2.5., UpperCI = X97.5., min, max) %>%
  mutate(across(.cols = c(p), ~ round(.x, 3))) %>% 
  mutate(across(.cols = c(Estimate:t_value), ~ round(.x, 2))) %>% 
  mutate(across(.cols = c(LowerCI:max), ~ round(.x, 2))) %>% 
  #mutate(across(Chi2:p, ~replace_na(.x, "")))%>%
mutate(p=replace(p, p==0, "<0.001"))

write.csv(model_table_mm1.latency , file = "saves/latency_mm1_output_table.csv")
```
...
latency_sc_data<- latency_sc_data%>%
  select(Subject, latency, condition, trial_w_con, first_condition)%>%
  full_join(first_choice_data%>%select(Subject, opaque_choice, condition, trial_w_con, first_condition))%>%
  mutate(choice = fct_recode(as.factor(opaque_choice), "opaque" ="1", "clear" ="0"))
