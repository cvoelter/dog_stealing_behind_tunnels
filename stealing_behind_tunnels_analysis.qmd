...
next steps: latency analysis for the terriers, side biases.
## load packages
...
```{r}
table(latency_fc_data$Subject, latency_fc_data$trial)
...
latency_fc_data$z.trial<-as.vector(scale(latency_fc_data$trial_w_con, center = TRUE, scale=TRUE))
latency_fc_data$condition <- as.factor(latency_fc_data$condition)
levels(latency_fc_data$condition)
latency_fc_data$condition.c=as.vector(scale(as.numeric(latency_fc_data$condition==levels(as.factor(latency_fc_data$condition))[2]), center=TRUE, scale= FALSE))

latency_fc_data$choice.c=as.vector(scale(as.numeric(latency_fc_data$choice==levels(as.factor(latency_fc_data$choice))[2]), center=TRUE, scale= FALSE))




latency_fc_data$log_latency<-log(latency_fc_data$latency)
hist(latency_fc_data$latency)
max(latency_fc_data$latency)

hist(latency_fc_data$log_latency)
...
```{r}
drop1_mm1.latency <- drop1(mm1.latency, test="Chisq")%>%
  filter(!is.na(npar))%>%
  add_row(npar = rep(NA,1),  .before = 1)
drop1_mm1.latency
...
model_table_mm1.latency <- bind_cols(as.data.frame(summary(mm1.latency.reml)$coefficients),
                         #drop1_mm1.yarbus.signaling,
                         mm1.latency_boot_ci) %>% #mm2_boot_ci<-boot.full$ci.estimates
