...
load("stealing_behind_tunnels_different_sample_sizes.RData")
```
...
set.seed(1)
n.subject <- c(90, 100, 110, 120) # number subjects
...
for (j in 1:length(n.subject)) {
  ...
  start.data$trial <- c(rep(c(1,2),n.subject[j]/2), rep(c(2,1),n.subject[j]/2))
...
save.image("stealing_behind_tunnels_different_sample_sizes.RData")
```
...
p.con.power <- ggplot(data = lrt.data2, aes(x= as.factor(r.effect),y = proportion.sign.lrt.con, fill=as.factor(test.per))) +
  ...
  theme_few()+
  ggtitle("Different sample sizes (90 - 120), 1 trial per condition")+
...
ggsave(p.con.power, filename = "graphics/stealing_behind_tunnels_differentNs_power.png", scale = 0.7, height = 12, width = 16)
```
